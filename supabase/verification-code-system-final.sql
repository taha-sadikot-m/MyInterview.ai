-- Verification Code System Update - CORRECTED for existing profiles table
...
-- Add verification code fields to existing profiles table
DO $$
BEGIN
...
ON public.profiles USING btree (email_verification_code) TABLESPACE pg_default;

CREATE INDEX IF NOT EXISTS idx_profiles_password_reset_code 
ON public.profiles USING btree (password_reset_code) TABLESPACE pg_default;

-- ===============================
-- 2. UPDATE PROFILE CREATION TRIGGER
-- ===============================

-- Create or update the handle_new_user function to work with profiles table
...
DROP FUNCTION IF EXISTS public.ensure_user_setup(TEXT, TEXT, TEXT);
...
-- You can test the functions after running this script:
/*
-- Test generate verification code
SELECT public.generate_verification_code('test@example.com');

-- Test verify email code
SELECT * FROM public.verify_email_code('test@example.com', '123456');

-- Test ensure user setup
SELECT * FROM public.ensure_user_setup(
